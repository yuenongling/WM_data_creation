...
STATS_PATH = os.path.join(WM_DATA_PATH, "round_step_DLR", "stats")
WALL_GEO_PATH = os.path.join(STATS_PATH, "ramp_profile_data.csv")
WALL_DATA_PATH = os.path.join(STATS_PATH, "wall_dns.csv")
...
def load_cf_cp_data():
    ...
    cp_data = np.loadtxt(os.path.join(STATS_PATH, "Cp.dat"), comments='#', skiprows=4)
    cf_data = np.loadtxt(os.path.join(STATS_PATH, "Cf.dat"), comments='#', skiprows=4)

    return cf_data, cp_data
...
def process_and_save_region_data(
    ...
    save_to_hdf5(data_dict, f"round_step_data")
...
# --- Load Wall Friction/Pressure Data ---
# Use numpy.loadtxt, skipping comments (#) and reading only columns 0 (X) and 1 (Y0)
wall_data = np.loadtxt(WALL_DATA_PATH , skiprows=1, delimiter=',')
x_cpcf = wall_data[:, 0] # Streamwise coordinates of wall points
cp_data = wall_data[:, 1] # Wall-normal coordinates of wall points
cf_data = wall_data[:, 2] # Friction coefficient
print(f"Loaded wall data shape: {wall_data.shape}")

# Load coordinates
coord = np.loadtxt(WALL_GEO_PATH, skiprows=1, delimiter=",")
X = coord[:, 0]
Y = coord[:, 1]
...
cf_interpolated = interpolate_values(x_normal[:, 0], x_cpcf, cf_data)
