...
def extract_cp_data(filename):
    """Extract x and cp_orig from cp.expnew.dat"""
    with open(filename, 'r') as f:
        file_content = f.read()
    ...
    # Find the variables line and get data
    x_values = []
    cp_orig_values = []
    ...
    for line in data_lines:
        ...
        if reading_data:
            ...
            if len(values) >= 3:
                x = float(values[0])
                cp_orig = float(values[2])  # cp_orig is the third column
                
                x_values.append(x)
                cp_orig_values.append(cp_orig)
    
    return pd.DataFrame({'x': x_values, 'cp_orig': cp_orig_values})
...
def extract_profile_data(filename):
    ...
    u_index = 2  # Default index if not found
...
def main():
    ...
    cf_data = extract_cf_data(args.cf_file)
    cp_data = extract_cp_data(args.cp_file)
    profile_data = extract_profile_data(args.profile_file)
    
    # Save to CSV files
    cf_data.to_csv(f"{args.output}_cf.csv", index=False)
    cp_data.to_csv(f"{args.output}_cp.csv", index=False)
    profile_data.to_csv(f"{args.output}_profile.csv", index=False)
