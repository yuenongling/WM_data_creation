...
# --- Configuration ---
csv_filename = './10M/field/field_data_10M.csv' # From previous step
wall_data_filename = './10M/airfoil.txt'
output_profiles_filename = './10M/wall_normal_profiles.pkl'
output_dir = "./stats/"
...
import pandas
print(f"Loading reshaped data from: {csv_filename}")
loaded_data = pandas.read_csv(csv_filename)

# Only care about the suction side
loaded_data = loaded_data[loaded_data['Y'] > 0.0]

x_coords_2d = loaded_data['X'].to_numpy()      # Shape (J, I) -> (2304, 385)
y_coords_2d = loaded_data['Y'].to_numpy()      # Shape (J, I)
mean_u_2d = loaded_data['F1V2'].to_numpy()     # Shape (J, I)
mean_v_2d = loaded_data['F1V3'].to_numpy()     # Shape (J, I)
mean_p_2d = loaded_data['F1V4'].to_numpy()     # Shape (J, I)
...
wall_data = np.loadtxt(wall_data_filename, skiprows=1)
wall_data = wall_data[wall_data[:, 1] > 0.0] # Filter out negative Y values; only for suction side
...
points = np.column_stack((x_coords_2d, y_coords_2d))
...
max_normal_dist = 0.2 # Max distance to extend normal line (adjust based on channel height, Y1=2)
