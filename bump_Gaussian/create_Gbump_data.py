...
def load_cf_cp_delta_data():
    ...
    cf_data = pd.read_csv(
        os.path.join(STATS_PATH, "Cf.dat"),
        skiprows=2,
        delim_whitespace=True,
        header=None,
    ).to_numpy()
    cp_data = pd.read_csv(
        os.path.join(STATS_PATH, "Cp.dat"),
        skiprows=2,
        delim_whitespace=True,
        header=None,
    ).to_numpy()
    delta_data = np.loadtxt(
        os.path.join(STATS_PATH, "delta_2M.csv"), delimiter=","
    )
...
def process_and_save_region_data(
    ...
    save_to_hdf5(data_dict, f"gaussian_2M_data_{region}")
...
def main():
    ...
    # Load data
    data = load_bump_data("interpolated_data_all_vorz.pkl")
