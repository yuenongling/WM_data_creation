...
STATS_PATH = os.path.join(WM_DATA_PATH, "periodic_hill_Gloerfelt", "stats")
WALL_STATS_PATH = os.path.join(STATS_PATH, "hill_grid.csv")
OUTPUT_PATH = os.path.join(WM_DATA_PATH, "data")

for REYNOLDS_NUMBER in [10595]:
    UPPER_FRACTION = 0.18
    LOWER_FRACTION = 0.002
    UPPER_FRACTION_SEP = 0.1
    LOWER_FRACTION_SEP = 0.002
...
# --- Utility Functions ---
    ...
    def load_cf_data(cf_path):
        """Loads Cf, and delta data from text files."""

        coord = np.loadtxt(WALL_STATS_PATH, delimiter=',', comments='#', skiprows=1, usecols=(0, 1))

        X = coord[:, 0]
        Y = coord[:, 1]

        cf_data = np.loadtxt(os.path.join(STATS_PATH, cf_path), comments='#', skiprows=4)
    ...
    def process_and_save_region_data(
        ...
        for i in range(ind_start, ind_end):
            ...
            if np.isnan(up[i]):
                print(f"WARNING: Skipping point at x={x_normal[i,0]} due to NaN in up")
                continue
        ...
        save_to_hdf5(data_dict, f"ph_G_{REYNOLDS_NUMBER}_data")
...
# Load data
    data = load_bump_data(f"interpolated_profiles_Re{REYNOLDS_NUMBER}.pkl")
    X, Y, cf_data = load_cf_data(f"Cf_Re{REYNOLDS_NUMBER}.dat")
...
# WARNING: No direct cp data, use p interpolated on wall


    p_wall = np.loadtxt(STATS_PATH + f"/p_Re{REYNOLDS_NUMBER}.dat", delimiter=' ', skiprows=1)
    x_p = p_wall[:, 0]
    y_p = interpolate_values(x_p, X, Y)
